    ...
    public partial class UCUploadConfig : UserControl
    {
        ...
        private void LoadUploadConfig(){
            ...
            try
            {
                ...
                nudAutoReturnDelay.Value = Common._AutoReturnDelay;

                nudTime.Enabled = false;
                nudOrderCount.Enabled = false;
                nudNodeCount.Enabled = false;
                txtAddress.Enabled = false;
                cboDefaultProvince.Enabled = false;

                chbAddress.Checked = false;
                chbNodeCount.Checked = false;
                chbOrderCount.Checked = false;
                chbTime.Checked = false;
                chbDefaultProvince.Checked = false;

                cboDefaultProvince.DisplayMember = "Name";
                cboDefaultProvince.ValueMember = "Id";
                cboDefaultProvince.DataSource = FrmParent.ParentForm._AllRegion.Where(l => l.ParentId == 1).ToList();
                foreach (var item in cboDefaultProvince.Items)
                {
                    if (item.ToString() == Common._DefaultProvince)
                        cboDefaultProvince.SelectedItem = item;
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (chbDefaultProvince.Checked)
                {
                    Common._DefaultProvince = cboDefaultProvince.Text.Trim();
                    FrmParent.ParentForm.LoadLocalData();
                }

                if (chkAutoReturnDelay.Checked)
                    Common._AutoReturnDelay = Convert.ToInt32(nudAutoReturnDelay.Value);
        ...
        private void chbDefaultProvince_CheckStateChanged(object sender, EventArgs e)
        {
            cboDefaultProvince.Enabled = chbDefaultProvince.Checked;
            if (!cboDefaultProvince.Enabled)
                cboDefaultProvince.Text = Common._DefaultProvince;
