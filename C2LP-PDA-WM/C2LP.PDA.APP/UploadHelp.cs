...
namespace C2LP.PDA.APP
{
    public class UploadHelp
    {
        ...
        //public static bool _isSleep = false;
        ...
        private static void DoWork()
        {
            ...
            {
                //throw ex;
                FrmParent.ParentForm.SetNewInfo("上报失败:" + ex.Message, false);
            }
            finally
            {
                //if (!_isSleep)
                ...
                if (count == 0)
                {
                    ...
                    System.Threading.Thread.Sleep(97);
                //Thread.Sleep(Common._UploadCycle * 1000);
                //}
        ...
        /// 上传第三方运单号
        /// </summary>
        private static int UploadThirdOrder()
        {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    int consignorId = 0;
                    if ((row["consignorId"] is DBNull) == false)
                        consignorId = Convert.ToInt32(row["consignorId"]);
                    if (_isContinue)
                        return 0;
                    FrmParent.ParentForm.SetNewInfo("正在上报第三方运单" + hdOrder.RelationIdk__BackingField + ",请稍候...", null);
                    result = Common._PdaServer.UploadThirdOrderForCustomer(hdOrder, consignorId, true);
            ...
            finally
            {
                ...
                {
                    HuadongTmsOrderServer.DeleteUploadHuadongTmsOrder(new List<string>() { hdOrder.RelationIdk__BackingField });
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        int consignorId = 0;
                        int? parentStorageId = null;
                        if ((row["consignorId"] is DBNull) == false)
                            consignorId = Convert.ToInt32(row["consignorId"]);
                        if ((row["parentStorageId"] is DBNull) == false)
                            parentStorageId = Convert.ToInt32(row["parentStorageId"]);
                        if (_isContinue)
                            return 0;
                        FrmParent.ParentForm.SetNewInfo("正在上报节点" + msg + ",请稍候...", null);
                        result = Common._PdaServer.UploadNodeForStorage(n, consignorId, true, parentStorageId, true);
                        break;//只处理1单
                    }
                }
                return list.Count;
        ...
        private static int UploadPic()
        {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    //result = Common._PdaServer.UploadPostbackForCustomer(picModel, postbackTime, true, picBytesList.ToArray(), consignorId, true);
                    if (_isContinue)
                        return 0;
                    result = Common._PdaServer.UploadPostbackForCustomers(picModel, postbackTime, true, picBytesList.ToArray(), consignorId, true);
                    return 1;
                }
                else
                    return 0;
