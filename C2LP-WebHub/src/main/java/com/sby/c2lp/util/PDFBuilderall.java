 ...
 *
 */
public class PDFBuilderall extends AbstractITextPdfView {
    @Override
    protected void buildPdfDocument(Map<String, Object> pdfmodel, Document doc,
        ...
        List<Map<String,Object>> allpdfView = (List<Map<String,Object>>) pdfmodel.get("allpdfView");
        for (int n=0;n<allpdfView.size();n++){
            doc.newPage();
            Map<String, Object> model = allpdfView.get(n);
            List<Aiinfo> aiList = (List<Aiinfo>) model.get("aiList");
            List<Object[]> aiinfoHistoryList=(List<Object[]>) model.get("aiinfoHistoryList");
            String head = "冷库名/车牌号："+model.get("storageName")+"\n运单编号："+pdfmodel.get("number")+"\n发收货单位："+pdfmodel.get("senderOrg")+" 到 "+pdfmodel.get("receiverOrg")+"\n起止时间："+sdf.format(model.get("startTime"))+" 到 "+sdf.format(model.get("endTime"));
